...
sequenceDiagram
    ...
    participant SearchRoute as /api/restaurants/search
    participant EvalRoute as /api/restaurants/evaluate/stream
    participant Service as restaurant-search.ts
    participant Places as Places API Text Search
    participant Gemini as Gemini 構造化評価

    User->>UI: ヒアリング完了後に検索結果画面へ遷移
    UI->>SearchRoute: POST 検索条件(URL query stateから復元)
    SearchRoute->>Service: searchBaseRestaurants(condition, pagination)
    Service->>Places: searchPlacesByText(textQuery, latLng, pageSize)
    Places-->>Service: 店舗候補(名前・placeId・住所・座標・写真参照など)
    SearchRoute-->>UI: JSON Restaurant[](AI生成フィールドはnull)
    UI->>UI: 一覧・地図へ先行表示

    UI->>EvalRoute: POST 検索条件 + 表示中Restaurant[]
    EvalRoute->>Service: streamEvaluationsForRestaurants(condition, restaurants)
    EvalRoute-->>UI: NDJSON phase: evaluating
    UI->>UI: スケルトンを増やさず評価中ステータスを表示

    Service->>Gemini: streamText(Output.array)
    loop 評価が確定した店舗ごと
        Gemini-->>Service: RestaurantEvaluationResult
        EvalRoute-->>UI: NDJSON restaurant-evaluated
        UI->>UI: updateRestaurantで既存カードを更新
    end
    EvalRoute-->>UI: NDJSON done
```
...
1. 検索・評価型（Places API 施設検索 + Gemini 構造化評価）: ヒアリング条件に合いそうな店舗を探し、AI 評価まで含めた `Restaurant[]` を1回の検索でまとめて生成する。
   ...
   トリガーはヒアリング完了後の検索実行。受け口は画面を持たない resource route `app/routes/api.restaurants.search.tsx`（`/api/restaurants/search`）と `app/routes/api.restaurants.evaluate.stream.tsx`（`/api/restaurants/evaluate/stream`）の `action` で、`_layout.results.tsx`（`results-screen.tsx`）は URL query state から復元した検索条件を `fetch` の POST body として渡す。`mvp-cycle-5` では、検索条件は Jotai だけに閉じず `nuqs` による URL query state を復元元にする。外部 API サーバーは別途立てない。
   ...
   `app/routes/api.restaurants.search.stream.tsx`（`/api/restaurants/search/stream`）は後方互換の段階表示確認用として残っているが、現在の UI は施設検索 JSON と評価ストリームの分離フローを使う。`app/mocks/fixtures/restaurants-search.json` フィクスチャの再生成は、評価込みの `searchRestaurants` を使う既存スクリプトや一時的な直接呼び出しで行う（詳細は `docs/ARCHITECTURE.md`「mock mode」）。
