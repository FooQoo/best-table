...
検索結果画面の地図と、比較画面の最終候補パネル内の地図は Google Maps JavaScript API による実地図として実装済み。`@vis.gl/react-google-maps` の `APIProvider` / `Map` / `Marker` を `app/components/feature/maps/restaurant-map.tsx` に共通化し、`ResultsMap` と `FinalStorePanel` の両方から使う。
...
- ライブラリは Google Maps JavaScript API の React バインディングとして `@vis.gl/react-google-maps` を使う。実装時点の 1.9.0 では `<APIProvider>` / `<Map>` / `<Marker>` を使用している。
