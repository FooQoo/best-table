...
| URL | ルートモジュール | 状態 | 責務 |
| --- | --- | --- | --- |
| `/api/restaurants/evaluate/stream` | `app/routes/api.restaurants.evaluate.stream.tsx` | 実装済み | `/api/restaurants/search` で先に返した店舗基本情報を受け取り、Gemini 構造化評価だけを NDJSON（`phase` → `restaurant-evaluated` → `done`）で逐次返す。`/results` は既存カードを同じ `id` で更新する |
| `/api/restaurants/search` | `app/routes/api.restaurants.search.tsx` | 実装済み | 検索条件を受け取り、Places API による施設検索結果を AI 評価なしの `Restaurant[]` として JSON でまとめて返す。`/results` はこの応答を先に描画し、その後 `/api/restaurants/evaluate/stream` で AI 評価を反映する |
...
開発時は `pnpm dev:mock` を用意し、mock mode では `/api/restaurants/search` が Gemini・Places API を呼ばず、記録済みの API モック `Restaurant[]` から AI 評価フィールドを null 化した施設基本情報を返す。これにより Google Maps の UI 実装とカード連動を、AI 応答や API キーに依存せず確認できる。`/api/results/chat` は mock mode でも Gemini（`streamText`）を実際に呼び出す。地図コンテキスト相談は表示中の `Restaurant[]` を都度リクエストボディで受け取るだけで、Places 等の課金対象 API に依存しないため、mock 固有の分岐を持たない。

mock/real の切り替えは resource route（`app/routes/api.restaurants.search.tsx`）の `action` に `if (MODE === "mock")` を書かず、repository 層（`app/server/repositories/restaurant-search-repository.ts`）に閉じ込める。`RestaurantSearchRepository`（`searchBase(condition, pagination)` と `search(condition, pagination)`）というインターフェースを定義し、`/results` が使う `/api/restaurants/search` は `searchBase` でAI評価なしの施設基本情報を返す。`realRestaurantSearchRepository` は `app/server/services/restaurant-search.ts` の `searchBaseRestaurants` / `searchRestaurants` へ委譲し、`mockRestaurantSearchRepository` は `loadMockRestaurants` で読んだフィクスチャをページングする。`getRestaurantSearchRepository()` が `process.env.MODE` を見てどちらを返すか決めるため、resource route 側は mock か real かを意識しない。

この API モックは手書きの架空データではなく、実際の検索・評価フロー（銀座・重要顧客・「落ち着いて話せる」を重視）で得た評価済み結果を記録したもの。座標・住所・スコア・懸念点・`photoUrl`（Places の photo resource name への参照）はすべて実データであり、mock mode でも `StoreList` 等の表示ロジックを実運用と同じ形状で確認できる。

記録先は `app/mocks/fixtures/restaurants-search.json` で、`.gitignore` 対象（リポジトリには含めない）。`app/server/repositories/restaurant-search-repository.ts` の `loadMockRestaurants` がビルド時に `import` せず、リクエストのたびに `fs.readFileSync` でファイルを読み込む（API モックという性質上、静的 import でバンドルに焼き込まない）。ファイルが存在しない・壊れている場合は例外を投げず空配列にフォールバックするため、フィクスチャを生成していないクローンでも mock mode 自体は起動できる（検索結果が0件になるだけ）。フィクスチャの再生成は `pnpm dev`（`MODE=real`）を起動し、施設検索と評価ストリームを通した評価済み応答を保存し直す。
...
`mvp-cycle-2` ではデスクトップ幅を主眼にした外部配置を採用し、モバイル向けレイアウトは表示が破綻しない範囲に留めた。`mvp-cycle-7` では、その保留を回収し、スマホ幅では「リスト表示」「地図表示」の切替を採用する。ボトムシート型の本格的なモバイル地図アプリ体験までは作らず、既存の情報密度と比較導線を保ったまま、スマホ幅で操作不能にならないことを優先する。

### スマホ幅のレスポンシブ方針（mvp-cycle-7）

スマホ対応は表示レイヤーの責務とし、検索 API、AI API、Places API、ドメインモデルは変更しない。`/results` は引き続き `app/routes/_layout.results.tsx` から `ResultsScreen` を描画し、画面幅に応じた表示切替は `app/components/feature/results/` 配下で扱う。

実装方針:

- `/results` のデスクトップ幅は、従来どおり `StoreList` 左、`ResultsMap` 右の Flexbox 左右分割を維持する。
- スマホ幅では `ResultsScreen` がローカル state で `list` / `map` の表示モードを持ち、店舗一覧と地図を同時には表示しない。URL query state や Jotai には持たせない。
- 初期表示は `list` にする。地図を見たい場合だけ `map` に切り替える。
- `StoreList` はスマホ幅で `w-full` になり、デスクトップ幅では既存の固定幅を維持する。
- `ResultsMap` はスマホ幅でも表示対象になったときは高さを持つ。Google Maps が読み込めない場合も、一覧・比較・AI相談の状態は維持する。
- `StoreDetailPanel`、`ComparePanel`、`ResultsAiChat` はスマホ幅では画面幅を使ったオーバーレイとして表示する。デスクトップ幅では既存の地図エリア内オーバーレイを維持する。
- 比較表は `ComparePanel` 内部のスクロール領域だけ横スクロールを許可し、アプリ全体に横スクロールを発生させない。
- `/` と `/hearing` は同じルート・同じ状態モデルのまま、フォーム部品の折り返し・縦積み・固定高の緩和でスマホ幅に対応する。
