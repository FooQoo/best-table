...
- Google Maps JavaScript API（React バインディング用に `@vis.gl/react-google-maps` を導入済み。`mvp-cycle-2` で `/results` と最終候補パネルの地図表示へ接続する）
- Google Places API (New) は `mvp-cycle-2` で座標・住所・代表写真の解決に使う。呼び出しはサーバー側に閉じ、10件単位に制限する。
...
- 最終候補パネル（`final-store-panel.tsx`）: `phone` / `access` に加えて、`mvp-cycle-2` で `location` を使った最終候補地図を表示する。`location` がない場合は地図情報なしとして扱う。
- MAP: `mvp-cycle-2` では `Restaurant.location` を地図表示の正とする。`location` がある店舗だけマーカー表示し、`pos.top/left` の相対配置は旧プレースホルダー専用として段階的に使わない方向へ寄せる。
...
- `Place Photos (New)` の `/media` を呼び、表示に使う画像 URL を取得する。
- 写真取得は初回10件・追加10件の範囲に限定し、まだ表示していない候補の写真を先読みしない。
- `photoUrl` を取得できない場合は、既存の `StorePhotoPlaceholder`（`room` から `resolvePhotoPlaceholderLabel` で導出したラベル）にフォールバックする。
...
検索結果画面の地図（現状は `results-map.tsx` によるプレースホルダー）と、比較画面の最終候補パネル内の地図を Google Maps JavaScript API による実地図へ置き換える。`@vis.gl/react-google-maps` は依存関係として導入済みで、`mvp-cycle-2` で接続する。
...
開発時は `pnpm dev:mock` を用意し、mock mode では `/api/restaurants/search` が Gemini を呼ばず、座標付きの固定モック `Restaurant[]` を返す。これにより Google Maps の UI 実装とカード連動を、AI 応答や API キーに依存せず確認できる。

### 10件単位の取得

`mvp-cycle-2` では、検索結果を一度に30件取得しない。初回は10件だけ取得し、店舗一覧の最下部までスクロールした時点で追加10件を取得する。これにより、Places API の `location` / `formattedAddress` / `photos` / Place Photos 取得が不要な候補にまで先行して発生することを避ける。

- `/api/restaurants/search` は初回10件を返し、追加取得リクエストで次の10件を返す。
- 取得範囲は `limit` と `offset`、または Gemini 側の候補リストを保持したうえでのページング相当の仕組みで表現する。実装時点のデータ保持方法に合わせて決める。
...
- Google Maps の APIProvider や地図キー注入は、検索結果地図と最終候補地図で重複しないよう共通コンポーネントに寄せる。
...
- `pnpm dev` は `MODE=real`、`pnpm dev:mock` は `MODE=mock` を立てる。`MODE=mock` の場合だけ AI・座標解決の外部呼び出しを避ける。本番 build で mock mode が有効にならないよう、分岐はサーバー側の resource route に閉じる。
