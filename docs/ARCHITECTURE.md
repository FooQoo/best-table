...
店舗カード・比較表の代表写真は `Restaurant.photoUrl` に格納する。UoW-7 時点では、グラウンディング出力に写真参照が含まれる場合だけ解決する方針だったが、実際の応答には `uri` / `title` / `placeId` しか含まれなかった。
...
- 検索結果・比較サイドパネル: `app/components/feature/results/`
