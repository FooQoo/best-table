...
| `/api/photos/*` | `app/routes/api.photos.$.tsx` | Place Photos の photo resource name を受け取り、`photo-repository.ts` 経由で画像を取得して中継する（real: サーバー専用 API キーで Google から取得、mock: 記録済みマッピングへリダイレクト） |
...
開発時は `pnpm dev:mock` を用意し、mock mode では `/api/restaurants/search` が Gemini・Places Details を呼ばず、記録済みの API モック `Restaurant[]` を返す。これにより Google Maps の UI 実装とカード連動を、AI 応答や API キーに依存せず確認できる。
...
- `pnpm dev` は `MODE=real`、`pnpm dev:mock` は `MODE=mock` を立てる。`MODE=mock` の場合だけ AI・座標解決の外部呼び出しを避ける。本番 build で mock mode が有効にならないよう、分岐はサーバー側の `app/server/repositories/restaurant-search-repository.ts`（`getRestaurantSearchRepository`）に閉じる。resource route やクライアントは mock/real を意識しない。
