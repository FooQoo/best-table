...
- `BookingState`（`app/state/booking-context.tsx`）は、本ドキュメントの `BookingRequest` + `ComparisonSelection` の2集約に加え、UoW-7 で追加された `restaurants: Restaurant[]`（`/results` が取得した実検索結果）も同じ atom に持っている。`mvp-cycle-5` では `BookingRequest` 相当の検索・会食条件を URL query state の復元対象にし、`ComparisonSelection.compareIds` と `restaurants` は URL に保存しない画面内一時状態として扱う。将来 `app/domain/models/` に型を切り出す際は、この境界を型定義上も分離することを検討する。
