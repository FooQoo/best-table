...
| 送客（Referral） | 納得した候補を予約手段へ外部リンクで引き継ぐこと。一休掲載店は一休.comの店舗詳細ページへ、それ以外は Google Maps の場所ページへ送客する。プロダクトのゴールであり、アプリ内の確定・決定操作ではない。 |
| 一休掲載店マスタ（IkyuListing） | 一休.comレストランに掲載されている店舗の一覧データ。店舗詳細ページ URL と照合用の店舗属性（店名・電話番号・placeId 等）を持つ。`mvp-cycle-6` ではモック fixture。 |
| 店舗同定（Matching） | 施設検索で得た候補と一休掲載店マスタを、placeId・電話番号・店名+住所で照合し、同一店舗と判定する処理。照合できない店舗は除外せず、一休リンク無しで扱う。 |
...
`mvp-cycle-6` で、プロダクトのゴールを「納得した候補の予約手段への送客」と定義した。送客は Handoff 集約の復活ではなく、店舗探索・評価コンテキストが `Restaurant` に付与する読み取り専用の外部リンク（`Restaurant.ikyu`、および既存の Google Maps リンク）として表現する。送客の状態（どの店へ送客したか）をアプリ側で保持・追跡する集約は持たない。
...
// 店舗同定（照合）に使ったキーの種別。固定語彙。
type IkyuMatchKey = "placeId" | "phone" | "name-address";

// 一休.comレストランへの送客リンク。URL は一休掲載店マスタ由来の値のみを使い、
// AI・自由入力・文字列組み立てで生成しない。
type IkyuReferral = {
  url: string; // 一休.comレストランの店舗詳細ページ URL
...
type Restaurant = {
  ...
  // 一休掲載店マスタ由来。施設検索の直後に店舗同定（照合）し、一致した場合だけ埋める。
  // 照合できない店舗・マスタ読込失敗時は null のまま一覧・地図・比較に表示し続ける（除外しない）。
...
- **自由記述**にするフィールド: `id` / `placeId` / `name` / `area` / `address` / `phone` / `photoUrl` / `ikyu.url` / `access` / `budgetLabel` / `concerns[].text` / `matchingSummary` / `generatedAt`。実世界の値の種類が多い、または固有名詞・URL・タイムスタンプなど値そのものに意味があるため、決め打ちの語彙で表現できない。`area` / `budgetLabel` は当初 `AREA_REGIONS` / `BUDGET_STEPS` から導出する固定語彙型にする計画だったが、実装時にその移設を見送ったため（下記「現行実装との対応」）、素直な `string` にしている。
- **固定語彙として型定義**するフィールド: `genre`（`Genre`）、`room`（`RoomAvailability`）、`quiet` / `prestige` / `service`（`RatingSymbol`）、`evidence`（`EvidenceCategory`）、`confidence`、`ikyu.matchedBy`（`IkyuMatchKey`）。いずれもアプリ側で選択肢が決まっている、または3〜10段階程度の小さな語彙に正規化できる。
...
- `ikyu` は一休掲載店マスタとの店舗同定（照合）でのみ埋める。AI 生成・自由入力・文字列組み立てで一休 URL を作らない。照合できない場合・マスタ読込に失敗した場合は `null` のままにし、店舗自体は除外しない（`docs/RELIABILITY.md`・`docs/SECURITY.md`）。

### ドメインサービス

- `IkyuMatchingService`（概念、`mvp-cycle-6` で追加予定）: 施設検索で得た候補（`placeId`・電話番号・店名・住所）と一休掲載店マスタの各エントリを照合し、一致した候補に `IkyuReferral` を付与する純粋関数。照合キーの優先順は `placeId` → 電話番号（正規化済み） → 店名+住所。複数キーで矛盾する場合は一致とみなさない。照合できない候補には何も付与しない（除外もしない）。
...
- `IkyuListingRepository`（`mvp-cycle-6` で追加予定、実装は `app/server/repositories/`）: 一休掲載店マスタ（店舗詳細ページ URL と照合用属性の一覧）を取得する境界。`mvp-cycle-6` ではモック fixture を読むだけの実装とし、将来のバリューコマース提携データ・API に差し替えられるインターフェースにする（`docs/plans/mvp-cycle-6/PLANS.md` の調査メモを参照）。読み込みに失敗した場合は空のマスタとして扱い、検索自体は失敗させない。
...
| `IkyuMatchKey` の語彙 | `app/domain/models/restaurant.ts`（`mvp-cycle-6` で追加予定） | `IkyuReferral.matchedBy` | 店舗同定の照合キー種別。AI 生成には使わない（zod スキーマ対象外） |
