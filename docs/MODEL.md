...
| 代表写真 | 店舗カードや最終候補パネルで表示する1枚の店舗写真。取得できない場合はプレースホルダーに戻す。 |
...
- `Store`（`app/mocks/data.ts`）は `Restaurant & { pos, photoPlaceholderLabel }` というプロトタイプ専用の拡張型として現在も残っている（`docs/ARCHITECTURE.md`「店舗データモデル」）。`mvp-cycle-2` では地図表示・住所・代表写真の正を `Restaurant.location` / `address` / `photoUrl` に寄せ、mock mode でも同じ形の `Restaurant[]` を `/api/restaurants/search` から返す。
