...
export function StoreList({ stores, compareIds, onToggleCompare }: StoreListProps) {
  const t = getTheme();
  const compareCount = compareIds.length;

  return (
    <div className="w-[400px] flex-none overflow-y-auto p-6 flex flex-col gap-4 bg-[#f7f4ee]">
      <div className="font-bold text-[15px]">接待安全度の高い順・{stores.length}件</div>
      {stores.map((store) => {
        ...
        return (
          ...
          >
            <div className="flex gap-3">
              <StorePhotoPlaceholder label={store.photo} className="w-20 h-20 flex-none" />
